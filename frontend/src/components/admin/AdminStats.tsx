...
export function AdminStats() {
  ...
  const mtbfData = data.mtbf
      ...
      .map((m) => ({ name: m.equipmentName ?? `기자재 #${m.equipmentId}`, value: m.mtbfDays as number }));
  const engData = data.engineerStats.map((e) => ({ name: e.engineerName ?? `엔지니어 #${e.engineerId}`, value: e.repairCount }));
  const topFailData = data.topFailingEquipment.map((m) => ({ name: m.equipmentName ?? `기자재 #${m.equipmentId}`, value: m.failureCount }));
