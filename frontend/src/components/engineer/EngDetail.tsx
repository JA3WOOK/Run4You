...
interface Props {
  asRequestId: number;
  onBack: () => void;
  onAccepted: (assignmentId: number, ctx: { asRequestId: number; equipmentId: number }) => void;
}
...
export function EngDetail({ asRequestId, onBack, onAccepted }: Props) {
  ...
  const handleAccept = async () => {
    try {
      const result = await accept(); // 이제 AcceptResult 객체
      onAccepted(result.assignmentId, {
        asRequestId: result.asRequestId,
